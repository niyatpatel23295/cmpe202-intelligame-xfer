...
2. The team will agree on the features that are going to be implemented in the game. After that every team member will study one or two of those feature(s), and decide using which design pattern that option should be implemented. We are also going to discuss the design and architechture of the project and UML diagrams, but the more detailed discussion and decision will take place in Week#3.

3. One obstacle was not having much experience with possible use cases of each design pattern, that made it difficult to decide on game features, and their implementation. We decided to address this challenge by studying several design patterns in detail with practical approach, and familiarizing ourselves with common use cases of each pattern.
